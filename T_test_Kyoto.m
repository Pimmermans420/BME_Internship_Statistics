%% CBCT Time Stability – T-tests Internship Kyoto Phantom
clear; clc; close all;

% Location of file
filename = "C:\Users\pimja\OneDrive\Documents\Pim UTwente\Pim Master\IVD track\Internship\MST\Metingen\DATA\QA_Results_Analysis\Kyoto_Results_MATLAB.xlsx";
...
for s = 1:numel(sheets)
    ...
    for c = 1:numel(comparisons)
        ...
        % Check for normality
        [~, p_norm1] = kstest((data1 - mean(data1)) / std(data1));
        [~, p_norm2] = kstest((data2 - mean(data2)) / std(data2));
